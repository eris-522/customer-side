...
export default function MyInquiriesPage() {
    ...
    const fetchInquiries = async () => {
      try {
        ...
        if (bookingsData && bookingsData.length > 0) {
          const { data: pkgData } = await supabase.from("packages").select("id, name");
          
          const merged = bookingsData.map((b) => ({
            ...b,
            packages: pkgData?.find((p) => String(p.id) === String(b.package_id)) || null,
          })).sort((a, b) => {
        ...
        setErrorMsg(err.message || "An unexpected error occurred while fetching your data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const confirmCancelBooking = async () => {
    ...
    const finalReason = cancelReason.trim() || "Cancelled by customer";
    try {
      ...
      setBookings((prev) =>
        prev.map((b) => (b.id === cancelModalId ? { ...b, status: "Cancelled", cancellation_reason: finalReason } : b))
      );
    } catch (err: any) {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    if (filter === 'Confirmed') return ['accepted', 'approved', 'confirmed'].includes(status);
    if (filter === 'Cancelled') return ['cancelled', 'declined', 'rejected'].includes(status);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {errorMsg && (
          <div className="bg-red-500/10 border border-red-500/20 p-4 mb-8 rounded-sm text-center max-w-2xl mx-auto">
             <p className="text-red-400 font-bold mb-1">Could not load bookings</p>
        ...
        {filteredBookings.length === 0 ? (
          ...
          >
            ...
            <div className="w-24 h-24 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-8 relative">
              ...
              <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-[#0a0a0a] rounded-full border border-white/10 flex items-center justify-center">
                <span className="text-gold-400 text-lg font-serif italic">0</span>
              </div>
            </div>
            ...
            {filter === 'All' ? (
              <Link to="/booking" className="gold-gradient text-black px-12 py-4 font-bold tracking-widest uppercase text-xs hover:brightness-110 transition-all inline-flex items-center gap-3 shadow-[0_0_30px_rgba(197,160,89,0.15)]">
                Start a Booking <ChevronRight size={16} />
              </Link>
            ) : (
              <button onClick={() => setFilter('All')} className="border border-white/20 text-white hover:border-gold-400 hover:text-gold-400 px-10 py-3 font-bold tracking-widest uppercase text-xs transition-all">
                View All Inquiries
              </button>
            )}
          </motion.div>
          ...
          <div className="grid gap-6">
            {filteredBookings.map((booking, i) => {
              ...
              const isCancelled = ['cancelled', 'declined', 'rejected'].includes(statusLower);
              const isApproved = ['accepted', 'approved', 'confirmed'].includes(statusLower);
              const reason = booking.reason || booking.reject_reason || booking.cancellation_reason || booking.admin_notes;
              ...
              return (
                ...
                >
                  <div className={`absolute top-0 left-0 w-1 h-full ${isCancelled ? 'bg-red-500' : isApproved ? 'bg-green-500' : 'bg-yellow-500'}`} />
                  <div className="space-y-5 flex-1 w-full">
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 text-[10px] uppercase tracking-widest font-bold border ${isCancelled ? 'border-red-500/50 text-red-500 bg-red-500/10' : isApproved ? 'border-green-500/50 text-green-500 bg-green-500/10' : 'border-yellow-500/50 text-yellow-400 bg-yellow-500/10'}`}>
                        {status}
                      </span>
                      <span className="text-xs text-white/40 font-mono">ID: {booking.id ? String(booking.id).split("-")[0] : 'N/A'}</span>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                      <div className="flex items-start gap-3 text-sm text-white/80">
                        <Calendar size={16} className="text-gold-400 shrink-0 mt-0.5" />
                        <span className="font-medium tracking-wide">{booking.event_date || 'Date TBD'} <br/><span className="text-white/50 text-xs">{booking.event_time || ''}</span></span>
                      </div>
                      <div className="flex items-start gap-3 text-sm text-white/80">
                        <MapPin size={16} className="text-gold-400 shrink-0 mt-0.5" />
                        <span className="font-medium tracking-wide leading-relaxed line-clamp-2">{booking.event_location || 'Location TBD'}</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-white/80">
                        <Package size={16} className="text-gold-400 shrink-0" />
                        <span className="font-medium tracking-wide">{booking.packages?.name || "Custom Package"}</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-white/80">
                        <Users size={16} className="text-gold-400 shrink-0" />
                        <span className="font-medium tracking-wide">{booking.guest_count || 0} Guests</span>
                      </div>
                    </div>
                    ...
                    {statusLower === 'pending' && (
                      <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 mt-4 rounded-sm">
                        <p className="text-sm text-yellow-400 font-medium">
                          Your booking request is currently pending and under review.
                        </p>
                      </div>
                    )}
        ...
        {cancelModalId && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-3xl font-serif text-white mb-2 italic">Cancel <span className="text-red-400">Booking</span></h3>
              <p className="text-sm text-white/60 mb-6 font-medium leading-relaxed">
                Are you sure you want to cancel this booking request? This action cannot be undone.
              </p>
              
              <div className="mb-8 text-left">
                <label className="text-sm font-bold text-white/80 mb-2 block">Reason for Cancellation (Optional)</label>
                <textarea
                  className="w-full bg-white/5 border border-white/10 p-4 text-sm focus:outline-none focus:border-red-500/50 transition-all resize-none placeholder:text-white/30"
                  rows={3}
              ...
              <div className="flex gap-4">
                <button onClick={() => { setCancelModalId(null); setCancelReason(""); }} disabled={cancellingId !== null} className="flex-1 py-3 border border-white/10 text-white/70 hover:text-white hover:border-white/30 transition-all text-sm font-bold tracking-wide">
                  Keep Booking
                </button>
                <button onClick={confirmCancelBooking} disabled={cancellingId !== null} className="flex-1 py-3 bg-red-500/10 border border-red-500/50 text-red-400 font-bold tracking-wide text-sm hover:bg-red-500/20 transition-all disabled:opacity-50">
